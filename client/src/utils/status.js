...
const status = [
  ...
  "The One Ring", "Pulitzer Prize In Pixels", "Nobel Prize In Pixels", "GOAT", "Palme d'Or", "Ballon d'Or", "No Lifer", "The Absolute Madman", "1337 H4X0R"
];
...
function getStatus(n) {
  ...
  for (var i=0, len=status.length; i<len; ++i) {
    const j = i + 1;
    const lower = (i < 4) ? i * 25 : (i - 3) * 100;
    const upper = (j < 4) ? j * 25 : (j - 3) * 100;
